...
about_selected(GtkMenuItem *item, gpointer user_data)
{
	...
	gtk_show_about_dialog(NULL,
			      ...
			      "copyright", "Copyright (c) 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022  OpenSS7 Corporation",
...
state_changed(WnckWindow *window, WnckWindowState changed, WnckWindowState state, gpointer data)
{
	...
	if (changed & WNCK_WINDOW_STATE_URGENT) {
		if (state & WNCK_WINDOW_STATE_URGENT) {
			ca_context_play_norm(ca, CaEventWindowChange, pl, "window-urgent", NULL, NULL);
		}
	}
	if (changed & WNCK_WINDOW_STATE_ABOVE) {
		if (state & WNCK_WINDOW_STATE_ABOVE) {
			ca_context_play_norm(ca, CaEventWindowChange, pl, "window-raise-above", NULL, NULL);
		}
	}
	if (changed & WNCK_WINDOW_STATE_BELOW) {
		if (state & WNCK_WINDOW_STATE_BELOW) {
			ca_context_play_norm(ca, CaEventWindowChange, pl, "window-lower-below", NULL, NULL);
		}
	}
	if (changed & WNCK_WINDOW_STATE_FOCUSED) {
		if (state & WNCK_WINDOW_STATE_FOCUSED) {
			if (window != wnck_screen_get_active_window(xscr->wnck)) {
				ca_context_play_norm(ca, CaEventWindowChange, pl, "window-inactive-click", NULL, NULL);
			}
		}
	}
...
window_closed(WnckScreen *wnck, WnckWindow *window, gpointer user)
{
	...
	if ((r = ca_proplist_setf(pl, CA_PROP_WINDOW_X11_XID, "%lu", (unsigned long) wnck_window_get_xid(window))) < 0)
		EPRINTF("Cannot set window xid: %s\n", ca_strerror(r));
	ca_context_play_norm(ca, CaEventWindowChange, pl, "window-close", NULL, NULL);
	ca_proplist_destroy(pl);
}
...
window_handler_PropertyNotify(XEvent *xev, XdeScreen *xscr)
{
	...
	if (atom == _XA_NET_WM_MOVING) {
		...
		if (xev->xproperty.state == PropertyDelete) {
			ca_context_play_norm(ca, CaEventWindowChange, pl, "window-move-end", NULL, NULL);
		} else {
			Atom actual = None;
			int format = 0;
		...
		ca_proplist_sets(pl, CA_PROP_CANBERRA_CACHE_CONTROL, "volatile");
		if (xev->xproperty.state == PropertyDelete) {
			ca_context_play_norm(ca, CaEventWindowChange, pl, "window-resize-end", NULL, NULL);
		} else {
			Atom actual = None;
			int format = 0;
			unsigned long nitems = 0, after = 0;
...
window_opened(WnckScreen *wnck, WnckWindow *window, gpointer user)
{
	...
	if ((r = ca_proplist_setf(pl, CA_PROP_WINDOW_X11_XID, "%lu", (unsigned long) wnck_window_get_xid(window))) < 0)
		EPRINTF("Cannot set window xid: %s\n", ca_strerror(r));
	ca_context_play_norm(ca, CaEventWindowChange, pl, "window-new", NULL, NULL);
	ca_proplist_destroy(pl);
}
...
term_signal_handler(gpointer data)
{
	...
	if (!already_exiting) {
		...
		id = "desktop-logout";
		if ((r = ca_context_play_norm(ca, CaEventWindowManager, pl, id, &desktop_logout_cb, pl)) != CA_SUCCESS)
...
void
inhibit_done(ca_context *ca, uint32_t id, int error, void *data)
{
	int *fdp = data;
...
void
take_inhibit_lock(GDBusProxy *sd, const char *what, int *fdp)
{
	GError *err = NULL;
	GVariant *result, *handle;
	gchar *descrip;

	if (*fdp >= 0)
		return;
	descrip = g_strdup_printf("Play sound before %s", what);

	result = g_dbus_proxy_call_sync(sd, "Inhibit",
					g_variant_new("(ssss)", what, "xde-sound", descrip, "delay"),
					G_DBUS_CALL_FLAGS_NONE, -1, NULL, &err);
	g_free(descrip);
	if (result) {
		GVariantIter iter;

		DPRINTF(1, "received call result %s( %s )\n", "Inhibit", g_variant_get_type_string(result));
		g_variant_iter_init(&iter, result);
		if ((handle = g_variant_iter_next_value(&iter))) {
			*fdp = g_variant_get_handle(handle);
			g_variant_unref(handle);
		} else {
			EPRINTF("no result for call %s\n", "Ihibit");
		}
	} else {
		EPRINTF("cannot inhibit %s; %s\n", what, err ? err->message : NULL);
		*fdp = -1;
		g_clear_error(&err);
	}
...
void
on_sd_proxy_manager_signal(GDBusProxy *proxy, gchar *sender_name, gchar
			   *signal_name, GVariant *parameters, gpointer user_data)
{
	...
	if (!strcmp(signal_name, "PrepareForSleep")) {
		...
		if ((value = g_variant_iter_next_value(&iter))) {
			ca_context_cancel_norm(ca, CaEventSleepSuspend);
			ca_proplist_create(&pl);
			ca_proplist_sets(pl, CA_PROP_CANBERRA_CACHE_CONTROL, "never");
			flag = g_variant_get_boolean(value);
			g_variant_unref(value);
			if (flag) {
				err = ca_context_play_norm(ca, CaEventSleepSuspend, pl, "suspend-start", &inhibit_done, &fd_sleep);
				ca_proplist_destroy(pl);
				if (err != CA_SUCCESS) {
					inhibit_done(ca, CaEventSleepSuspend, err, &fd_sleep);
				}
			} else {
				err = ca_context_play_norm(ca, CaEventSleepSuspend, pl, "suspend-resume", NULL, NULL);
				ca_proplist_destroy(pl);
			}
		}
	}
	if (!strcmp(signal_name, "PrepareForShutdown")) {
		...
		if ((value = g_variant_iter_next_value(&iter))) {
			...
			g_variant_unref(value);
			if (flag) {
				err = ca_context_play_norm(ca, CaEventSleepSuspend, pl, "system-shutdown", &inhibit_done, &fd_shutdown);
				ca_proplist_destroy(pl);
				if (err != CA_SUCCESS) {
					inhibit_done(ca, CaEventSleepSuspend, err, &fd_shutdown);
				}
			} else {
				ca_context_play_norm(ca, CaEventSleepSuspend, pl, "system-bootup", NULL, NULL);
				ca_proplist_destroy(pl);
			}
		}
	}
...
setup_systemd(void)
{
	...
	if (!(sd_manager = g_dbus_proxy_new_for_bus_sync (G_BUS_TYPE_SYSTEM, 0,
					NULL,
	...
	g_signal_connect(G_OBJECT(sd_manager), "g-signal", G_CALLBACK(on_sd_proxy_manager_signal), NULL);

	take_inhibit_lock(sd_manager, "sleep", &fd_sleep);
	take_inhibit_lock(sd_manager, "shutdown", &fd_shutdown);
}
...
void
on_up_manager_proxy_signal(GDBusProxy *proxy, gchar *sender_name, gchar
		*signal_name, GVariant *parameters, gpointer user_data)
{
	(void) proxy;
	(void) sender_name;
	...
	while ((prop = g_variant_iter_next_value(&iter))) {
		if (g_variant_is_container(prop)) {
			...
			if (id) {
				...
				ca_proplist_create(&pl);
				ca_proplist_sets(pl, CA_PROP_CANBERRA_CACHE_CONTROL, "never");
				ca_context_play_norm(ca, CaEventPowerChanged, pl, id, NULL, NULL);
				ca_proplist_destroy(pl);
			}
...
setup_upower(void)
{
	...
	if (!(up_manager = g_dbus_proxy_new_for_bus_sync(G_BUS_TYPE_SYSTEM, 0,
					NULL,
	...
	g_signal_connect(G_OBJECT(up_manager), "g-signal",
			G_CALLBACK(on_up_manager_proxy_signal), NULL);
	g_signal_connect(G_OBJECT(up_manager), "g-properties-changed",
			G_CALLBACK(on_up_manager_proxy_props_changed), NULL);
	if (!(up_display = g_dbus_proxy_new_for_bus_sync(G_BUS_TYPE_SYSTEM, 0,
					NULL,
					"org.freedesktop.UPower",
