...
Provides for themeability of the dock following the XDE theme.
...
Provides the ability to autostart dock applications when B<xde-dock>
starts.
...
options should not be concatentated.
...
Only one of the following command options should be specified.  When no
...
General option do not affect the major operating mode of the program.
...
Print debuggin information on standard error while running.  When
...
reparenting, EWMH/NetWM compliant window managers; however, not that
...
L<fluxbox(1)> provides a dock for dock applicaitons that it calls the
I<slit>.  Only a rudimentary arrangement of dock apps is provided by the
...
from the L<fluxbox(1)> I<slit>.  B<xde-dock> still respects changes made
to the L<fluxbox(1)> I<slit> configuration so that panel menu controls
still function.
...
B<xde-dock> provides an enhanced docking exprience for L<blackbox(1)>,
by identifying dock apps, unmapping them, and then reparenting them away
from the L<blackbox(1)> I<slit>.  B<xde-dock> still respects changes
made to the L<blackbox(1)> I<slit> configuration so that panel menu
controls still function.

=item L<openbox(1)>

L<openbox(1)> provides a dock for dock applicaitons that it calls the
...
by identifying dock apps, unmapping them, and then reparenting them away
from the L<openbox(1)> I<dock>.  B<xde-dock> still respects changes made
to the L<openbox(1)> I<dock> configuration so that L<obconf(1)> controls
will still function.
...
L<icewm(1)> does not understand mapping windows in the C<WithdrawnState>:
it interprets the C<WithdrawnState> as a don't-care state.

B<xde-dock> provides a consistent docking experience for L<icewm(1)>,
by identifying dock apps, unmapping them, and then reparenting them away
from the L<icewm(1)>.
...
dock apps will often appear in their own window becuase L<jwm(1)> does
not understand mapping windows in the C<WithdrawnState> and does not
display dock applications at all.

B<xde-dock> provides a consistent docking experience for L<jwm(1)>, by
identifying dock apps, unmapping them, and then reparenting them away
from the L<jwm(1)>.
...
of dock apps in tiles the way that others (L<fluxbox(1)>,
...
identifying dock apps, unmapping them, and then reparenting them away
from the L<pekwm(1)> I<harbour>.  B<xde-dock> still repsects changes
made to the L<pekwm(1)> I<harbour> configuration so that root menu
controls still function.
...
I<wharf>.  It provides for configured arrangement of dock apps; however,
docking is not automatic and specific dock applications must be
