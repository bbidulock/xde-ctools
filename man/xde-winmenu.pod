...
When specified, the following optios alter the primary operating mode of
the program:
...
Post a popup menu.  Depending on the setting of the B<--button> option,
this will either post the popup menu at the current pointer position, or
centered on the window to which it corresponds.
...
for which to popup a menu.  Can also specified with B<--which>.

=item B<--where>, B<-W> {B<pointer>|B<center>|B<topleft>|B<icongeom>}

Specifies the position for the popup menu.  This can be one of the
...
otherwise, defautls to the top left corner of the monitor in which the
...
wnidow (inside the decorative boder).
