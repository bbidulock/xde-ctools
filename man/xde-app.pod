...
xde-app - XDG application launch dialog for XDE
...
B<xde-app> recognizes the following options:
...
command option is specified, B<xde-app> will assume normal operation is
requested and launch the XDG launcher command dialog.

B<xde-app> provides the following command options:
...
For B<xde-app>, this command is assumed if no other command is provided.

=item B<--binary>, B<-b>

Run the dialog as a binary command instead of an XDG launcher.
For L<xde-run(1)>, this command is assumed if no other command is provided.
...
B<xde-app> provides the following general options:
...
When in XDG launcher mode, runt the command dialog with the specified
...
B<xde-app> uses the following files:
...
B<xde-app> does not perform its own XDG startup notification.
Instead, it uses the L<xdg-launch(1)> command to perform this function.

B<xde-app> and L<xde-run(1)> differ only in the default behaviour
(whether to run a binary command dialog or an XDG application launcher).

=head1 BUGS

B<xde-app> is quite stable, performs well and has no known bugs.
