...
B<xde-wait> [I<OPTIONS>] [-c I<command and arguments>]

=head1 DESCRIPTION

B<xde-wait> is a simple C<C> language program that can be used to wait
for a window manager, system tray or pager before running a command or
exiting.
...
=item B<-c>, B<--command> I<command and arguments>
...
B<xde-wait> will L<fork(3)> and L<execve(3)> the arguments in a similar
fashion to C<sh -c>.
...
continuing.  This option will only wait I<MILLISECONDS> before
proceeding.
...
The common procedure when starting light-weight window managers is to
start a shell task in the background that waits for two seconds before
starting applications like so:
...
full desktop environment to start.  With B<xde-wait>, you would use the
following:
