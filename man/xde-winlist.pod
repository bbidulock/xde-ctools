...
Specifies the current window for which the pop up the menu.  This can be
...
client with the keyboard focus) as the current window.  This is the with
a B<_NET_WM_STATE_FOCUSED> atom set in B<_NET_WM_STATE>, or the client
