...
Rather than simply recreate the experience of the WindowMaker dock using
a WindowMaker, Blackbox or AfterStep style, B<xde-clip> performs some of
the functions of a launcher, a panel, and recent/frequent applications
list.
...
L<fluxbox(1)> provides a dock for dock applications (in blackbox style)
that it calls the I<slit>; however, it does not provide a clip.
L<fluxbox(1)> provides an internal panel that displays current
applications in a task bar.

=item L<blackbox(1)>

L<blackbox(1)> provides a dock for dock applications (in blackbox style)
that it calls the I<slit>; however, it does not provide a clip.
...
L<openbox(1)> provides a dock for dock applications (in blackbox style)
that it calls the I<dock>; however, it does not provide a clip.
...
L<pekwm(1)> provides a dock for dock applications (in blackbox style)
that it calls the I<harbour>; however, it does not provide a clip.
L<pekwm(1)> does not provide a panel, but relies on external panel
programs.

=item L<wmaker(1)>

L<wmaker(1)> provides a dock for dock applications (in WindowMaker
...
L<fvwm(1)> provides a dock for dock applications (in AfterStep style)
that it calls the I<wharf>; however, it does not directly provide a
clip.
...
I wrote B<xde-clip> because there were many supported window managers
in the I<XDE> suite that did not support a window maker application
clip.  Once written, it was apparent that a more rich experience could
be provided to users of those window managers that do support a clip (or
