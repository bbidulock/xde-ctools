...
X11 Session Manager.  The following options are used X session
management:
...
While the feedback pop-up is display, the mouse can be used to select a
desktop.  When selected the window will not drop until the mouse leaves
the pop-up, a modifier is released, or a keypress is made that does not
