...
Provides a workspace menu for window managers that do not provide on
...
=item B<--format=>blackbox

L<blackbox(1)> provides a workspace menu that follow a specific format.
This format mimics the L<blackbox(1)> workspace menu format.
