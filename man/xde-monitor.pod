...
application.  To perform these functions, B<xde-monitor> takes the
following steps:
...
Provides the name of the program that launched the staring application.
Examined to determine the specific launcher that was used to launch the
application and work around potential defects or deficiencies in
specific launchers.

=item C<LAUNCHEE=>

Provides the name of the program being launched.  This is often either
the name of the application identifier of a desktop file, or the name of
the binary program being launched.
...
Provide a unique identifier of the launch instance.  This identifier is
...
on the leader window of a application that creates windows.  Examined
for comparison against the B<DESKTOP_STARTUP_ID> of processes along with
the B<_NET_STARTUP_ID> property on windows.
