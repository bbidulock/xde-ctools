...
When no command option is specified, or when invoked as B<xde-menu>,
normal operation is begun and a new instance is created.
This option causes an error when another instances of B<xde-menu> is
running.
...
running.  This option implies the B<--monitor> option.  This is
the default when B<xde-menu> is invoked as B<xde-menu-replace>.
