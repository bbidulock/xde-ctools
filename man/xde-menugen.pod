...
=item B<--rooo-menu>, B<-r> I<MENU>

Specify the location of the root menu file.  The default is calculated
using XDG denvironment variables (see L</ENVIRONMENT>), and defaults to
...
Specify the icon theme name ot use when generating icons.  The default
...
reference as an "Appications" menu.
...
directly or not.  This option will only be honored when the
